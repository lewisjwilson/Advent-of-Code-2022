...
# A given rucksack always has the same number of items in each of its two compartments, so the first half of the characters represent items 
...
for line in contents:
    ...
    pocket1 = line[:l]
    pocket2 = line[l:]
    for i in range(len(pocket1)):
        character = pocket1[i]
        if(character in pocket2):
            priority = priorities.find(character) + 1 # 0 indexing
            print(character, priority)
            pri_sum += priority
            break
