...
# 
# Unless, that is, you can design a replacement for the device's video system! It seems to be some kind of cathode-ray tube screen 
# and simple CPU that are both driven by a precise clock circuit. The clock circuit ticks at a constant rate; each tick is called a cycle.
# 
...
def check_signal():
    if(cycles in sig_check):
        signals.append(cycles*X)
    
X = 1
cycles = 0
sig_check = [20, 60, 100, 140, 180, 220]
signals = []
...
for s in range(len(signals)):
    print(f"Signal at {sig_check[s]}th cycle: {signals[s]}")
