...
#
# Additionally, nobody wrote down which item type corresponds to each group's badges. The only way to tell which item type is the right one 
...
# Evaluating 3 lines (rucksacks) at a time
while line < input_len:
    rucksack1 = contents[line]
    line += 1
    rucksack2 = contents[line]
    line += 1
    rucksack3 = contents[line]
    line += 1
    for c in rucksack1:
    ...
    for c in matches:
        priority = priorities.find(c) + 1
        pri_sum += priority
    print(matches)
    matches = set()
print(f"The sum of priorities is {pri_sum}")
